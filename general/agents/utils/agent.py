...
class Agent:
    ...
    def __init__(self, agent_name, agent_prompt, is_supervisor=False, model="gemini-2.0-flash-lite", temperature=0,
                 max_tokens=None,
        ...
        if not is_supervisor:
            if os.path.exists(agent_prompt):
                self.prompt = self.create_prompt(prompt=open(agent_prompt, "r").read())
            else:
                self.prompt = self.create_prompt(prompt=agent_prompt)
        else:
            if os.path.exists(agent_prompt):
